...
with tab_fixtures:
    ...
    for i, m in visible:
        ...
        with st.expander(label, expanded=False):
            ...
            m["team_a"] = st.text_input("Team A", m.get("team_a", ""), key=f"mta{i}")
            ...
            if label_a == CUSTOM_COLOR_LABEL:
                m["team_a_color"] = st.color_picker("Custom A", cur_a, key=f"mtac{i}")
            else:
                m["team_a_color"] = TEAM_COLORS[label_a]

            m["team_b"] = st.text_input("Team B", m.get("team_b", ""), key=f"mtb{i}")
            ...
            if label_b == CUSTOM_COLOR_LABEL:
                m["team_b_color"] = st.color_picker("Custom B", cur_b, key=f"mtbc{i}")
            else:
